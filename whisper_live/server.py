...
from whisper_live.transcriber import WhisperModel,Segment
try:
...
class TranscriptionServer:
    ...
    def initialize_client(
        ...
        try:
            ...
            logging.info("faster_whisper backend failed.")
            return
...
class ServeClientBase(object):
    ...
    def send_transcription_to_client(self, segments):
        ...
        try:
            ...
            # self.frames_np = np.empty(1, dtype=np.float32)
        except Exception as e:
            logging.info(f"[ERROR]: Sending data to client: {e}")
...
class ServeClientFasterWhisper(ServeClientBase):
    ...
    def handle_transcription_output(self, result, duration):
        ...
        """
        segments = []
        if len(result) and isinstance(result,list) and isinstance(result[0],Segment):
