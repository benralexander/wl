...
class Client:
    ...
    def __init__(
        ...
        self.ws_thread = threading.Thread(target=self.client_socket.run_forever, args=(transcript,))
    ...
    def process_segments(self, segments):
        ...
        try:
            ...
            logging.error(f"Add parameters to docstring={e}, and {e.args[0]}")
    ...
    def printing_func(self,transcript):
        ...
        text = []
        print(f"segs={transcript.qsize()}")
        for t in transcript.queue:
            text.append(t['text'])
        print("Thread finished.")

    def stop_thread(self,thread, timeout):
        thread.join(timeout)
        if thread.is_alive():
            print("Thread timed out, stopping...")
            # You can't forcefully terminate a thread in Python,
            # but you can use a flag to signal it to stop.

    def launch_data_exposing_thread(self):
        my_thread = threading.Thread(target=self.printing_func,args=(transcript,))
        my_thread.start()
        self.stop_thread(my_thread, 3)  # Stop the thread after 2 seconds
    ...
    def on_error(self, ws, error):
        ...
        if isinstance(error,dict):
            print(f'dict in on_error={dict}')
        print(f"[ERROR] WebSocket Error: {error}")
        self.server_error = True
        self.error_message = error
        utils.print_material(transcript)
        logging.info("connection error done")
    ...
    def on_close(self, ws, close_status_code, close_msg):
        ...
        self.recording = False
        self.waiting = False
        logging.info("connection close done")
        ...
        if self.server_backend == "faster_whisper":
            if not transcript.empty():
                if self.last_segment is not None:
                    transcript.put(self.last_segment)
                if transcript.qsize()>0 and self.last_segment and transcript.snapshot()[-1]["text"] != self.last_segment["text"]:
                    transcript.put(self.last_segment)
            utils.create_srt_file(transcript, output_path)
...
class TranscriptionTeeClient:
    ...
    def process_av_stream(self, container, stream_type, save_file=None):
        ...
        try:
            ...
            time.sleep(5)
            logging.info(f"we have {len(self.clients)} clients")
